...
export function Details() {
  ...
  useEffect(() => {
    const fetchPackageDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/package/detail/${packageId}`);
        
        // Log the response data for debugging
        console.log("API Response:", response.data);
        
        // Assuming the response data is an array, access the first element
        setPackageDetails(response.data[0]);  // Access the first object in the array
      } catch (error) {
  ...
  console.log("bigDiscription:", packageDetails.bigDiscription);
  ...
  if (!packageDetails.bigDiscription) console.log("Big Description is missing or undefined");
    ...
    <div>
      ...
      <Container className="my-5">
        ...
        <Container className="my-5">
          <Row className="align-items-start">
            <Col xs={12} md={12}>
              <p>{packageDetails.bigDiscription}</p>  {/* Big Description under Details */}
            </Col>
          </Row>
        </Container>
